...
# 한글 출력을 위한 PDF 클래스 (NanumGothic 폰트 필요)
class TransactionPDF(FPDF):
    def __init__(self):
        super().__init__()
        # 폰트 파일이 같은 디렉토리에 있어야 합니다. (예: NanumGothic.ttf)
        # self.add_font('NanumGothic', '', 'NanumGothic.ttf', uni=True)
        # self.set_font('NanumGothic', '', 10)
        
        # 테스트용 기본 폰트 (실제 사용 시 위 주석 해제 및 한글 폰트 적용 필수)
        self.set_font('Arial', '', 10)

    def header(self):
        self.set_font('Arial', 'B', 15) # 한글 적용 시 변경
...
def create_pdf(center_name, data_rows):
    ...
    # 공급자 / 공급받는자 기본 정보 (첫 번째 시트 양식 참고)
    pdf.cell(0, 10, f"Recipient: {center_name}", 0, 1, 'L')
    pdf.cell(0, 10, f"Date: {datetime.now().strftime('%Y-%m-%d')}", 0, 1, 'L')
    pdf.ln(5)
    
    # 테이블 헤더
    headers = ['Product Name', 'Qty', 'Unit Price', 'Total Amount']
    ...
    return pdf.output(dest='S').encode('latin-1') # 한글 처리 시 인코딩 확인

def main():
    st.set_page_config(page_title="명세서 PDF 자동화", layout="wide")
    st.title("📦 지점별 거래명세서 PDF 자동 생성기")
    st.write("엑셀 파일을 업로드하면 2번째 시트의 데이터를 바탕으로 지점별 명세서 PDF를 일괄 생성합니다.")
    ...
    if uploaded_file is not None:
        try:
            ...
            # 두 번째 시트 로드 (센터별 물량 데이터)
            # 헤더가 여러 줄일 수 있으므로 적절히 skip 설정 (보내주신 csv 기준)
            df_matrix = pd.read_excel(xls, sheet_name=sheet_names[1], header=2) 
            
            # 단가 정보 딕셔너리 (실제로는 첫 번째 시트에서 추출하거나 별도 매핑 필요)
            # 예시 단가 세팅
            price_dict = {
                "멘소래담 로션 75ml": 3780,
                "멘소래담 로션 100ml": 4590,
            ...
            if st.button("지점별 PDF 생성 및 압축"):
                # ZIP 파일을 메모리에 생성
                zip_buffer = io.BytesIO()
                
                with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_file:
                    # '제품명', '규격' 등을 제외한 실제 센터명(열)만 추출
                    ...
                    for center in center_columns:
                        ...
                        # 핵심 로직: 열 이름에 '할증'(또는 '힐증')이 포함되어 있는지 확인
                        ...
                        for index, row in df_matrix.iterrows():
                            ...
                            if pd.notna(qty) and str(qty).replace('.0','').isdigit() and int(qty) > 0:
                                ...
                                if is_bonus:
                                    ...
                                    unit_price = price_dict.get(product_name, 0) # 매핑된 단가 가져오기
                        ...
                        # 해당 지점에 배송할 물품이 1개라도 있다면 PDF 생성
                        if data_rows:
                            # 안전한 파일명을 위해 특수문자 치환
                ...
                st.balloons()
                st.success("PDF 변환 및 압축이 완료되었습니다!")
